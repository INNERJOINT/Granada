...
import type { HookDeps, HookInput, HookObjectOutput, HookRuntime } from '../types/hook.js';
...
export function extractCodexPatchedFiles(command: unknown): string[] {
...
export function normalizeCodexHookInputs(input: HookInput): HookInput[] {
  if (input.hook_event_name !== 'PostToolUse' || input.tool_name !== 'apply_patch') return [input];
...
function mergeOutputs(outputs: Array<HookObjectOutput | null | undefined>): HookObjectOutput | null {
  const defined = outputs.filter((value): value is HookObjectOutput => !!value);
...
function previewInput(input: HookInput): string {
  const toolInput = input.tool_input;
  if (!toolInput || typeof toolInput !== 'object') return JSON.stringify(input);
  const command = toolInput.command;
  if (typeof command !== 'string' || command.length <= TOOL_INPUT_PREVIEW_CHARS) return JSON.stringify(input);
...
async function run(runtime: HookRuntime): Promise<void> {
  ...
  if (handlerName !== 'enqueue-artifact' && handlerName !== 'drain-artifacts') return;
  ...
  if (handlerName === 'drain-artifacts') {
    ...
    if (output) writeJson(runtime, output);
    return;
  }

  const outputs: Array<HookObjectOutput | null | undefined> = [];
