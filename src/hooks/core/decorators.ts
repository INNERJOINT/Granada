...
function warningOutput(hookEventName: string, label: string, message: unknown): HookObjectOutput {
  return {
    hookSpecificOutput: {
      hookEventName,
...
export function withWarningBoundary(handler: HookHandler, options: { hookEventName?: string; label?: string } = {}): HookHandler {
