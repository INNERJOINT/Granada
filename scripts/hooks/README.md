# Claude Code Hooks 文档（Runtime / Input / Control）

## Runtime layout

The TypeScript source under `src/hooks/**` is compiled with `npm run build:hooks`. Claude Code loads the committed CommonJS adapter and ESM runtime through the repository hook manifest:
...
Claude Code uses its `command` plus `args` hook shape and `${CLAUDE_PLUGIN_ROOT}`. Malformed hook input and unknown routes intentionally exit quietly to avoid noisy host warnings.

Use `npm run typecheck:hooks`, `npm run build:hooks`, `npm test`, and `npm run verify:hooks-package` before release. Package verification executes the packed manifest-facing entrypoints outside the repository tree and checks their transitive `dist/**` imports.

## Artifact lifecycle

`.granada/**/*.md` artifact processing is intentionally deferred: `PostToolUse` only enqueues eligible candidates into `.granada/.hooks/artifact-queue/**`, and `Stop` drains the queue once per session to timestamp and translate final source content. Claude Code enqueues `Write|Edit` paths.

The queue keeps only the latest entry per source path in the active session, so repeated edits produce one timestamp/translation from final source content. `GRANADA_TRANSLATE_LANG` controls the target language and output suffix, defaulting to `zh` / `_zh.md`; `GRANADA_TRANSLATE_ENABLE` defaults to enabled and can be set to `0`, `false`, `no`, `off`, or `disabled` to timestamp without translating. `GRANADA_TRANSLATE_COMMAND` overrides the translation command. The default is `claude -p --model haiku --no-session-persistence`, preventing recursive hook execution.

The runtime queue uses collision-safe JSON entries, an atomic drain lock, and stale cleanup defaults of 24h for journal/failure entries and 15m for drain locks. Direct `timestamp-artifact` and `translate-artifact` routes remain available for debugging; direct `translate-artifact` continues to honor `artifacts-dirs`, while the `Stop` drain path uses the `.granada` runtime artifact policy.
