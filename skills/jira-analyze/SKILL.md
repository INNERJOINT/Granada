...
1. **Parse `{{ARGUMENTS}}`** as exactly one JIRA URL:
...
2. **JIRA MCP health check**:
   - JIRA: call `jira_get_issue(issue_key=<KEY>, fields="summary,status,assignee,priority,description")` — if it fails, abort with "mcp-atlassian unreachable. Check JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN env vars."
...
Agent(
  ...
  model="sonnet",
  prompt="**AOSP Project Resolved From Logs:** Use project `<resolved_project>` for ALL mcp__plugin_zaku_sourcepilot__* search calls. The project was resolved from `ro.build.display.id=<keyword>` via `mcp__plugin_zaku_sourcepilot__resolve_project_by_keyword`. Do NOT read `.granada/aosp-config.json` and do NOT ask the user for a project.
...
Search budget:
- Use only 1-2 precise mcp__plugin_zaku_sourcepilot__* queries.
...
4. **Post report as JIRA comment**: `jira_add_comment(issue_key=<KEY>, body=<redacted_report_content>)` — post the redacted report content as a comment on the JIRA issue. If this fails, warn but do not abort (the local report file is still available).
...
- **JIRA MCP unreachable** → abort with "mcp-atlassian unreachable. Check JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN env vars."
