...
Call `mcp__sourcepilot__list_projects()` to verify the MCP server is reachable and upstream is responding.
...
1. Run coarse exploration with a 2-round adaptive budget:
   - Round 1: spawn 2 `aosp-investigator` agents in parallel.
...
Spawn 3 `aosp-investigator` subagents in parallel. Each investigator searches independently but receives the same feature-first frame.

```
delegate(
  role="aosp-investigator",
  reasoning="medium",
  message="Investigate AOSP for a VENDOR feature meta-slice.
