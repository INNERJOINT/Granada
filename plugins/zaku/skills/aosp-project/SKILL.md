...
Call `mcp__sourcepilot__list_projects()` to verify the MCP server is reachable and fetch the project list in a single round trip.
...
Otherwise, use `request_user_input` to let the user pick from the project list. Include a "Keep current" option if a project is already configured, and a "Clear (search all)" option.
...
- `mcp__sourcepilot__list_projects`: Lists available AOSP projects from the remote MCP server. Doubles as the MCP health check.
...
- `request_user_input`: Interactive project selection
