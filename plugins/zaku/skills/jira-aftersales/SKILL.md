...
1. **Parse `<skill-arguments>`** to extract the issue key:
...
2. **MCP health check**: `mcp__atlassian__jira_get_issue(issue_key=<KEY>, fields="summary")` — if fails, abort with "mcp-atlassian is unavailable. Check the JIRA_URL and JIRA_PERSONAL_TOKEN environment variables."
...
- If local file not found: call `mcp__atlassian__jira_get_issue(issue_key=<KEY>, comment_limit=50)`
...
<!-- Design note: We intentionally do NOT auto-invoke jira-analyze via skill invocation mid-execution.
...
- If comments were already fetched in Tier 2, reuse them. Otherwise fetch: `mcp__atlassian__jira_get_issue(issue_key=<KEY>, comment_limit=50)`
...
delegate(
  role="executor",
  reasoning="medium",
  message="You are an aftersales-script conversion expert. Convert the technical root-cause analysis report into a customer-service script that support agents can copy and paste directly to users.
...
2. **Post as JIRA comment**: `mcp__atlassian__jira_add_comment(issue_key=<KEY>, body=<script_content>)` — if this fails, warn but do not abort (the local copy is still available).
...
<Error_Handling>
- **MCP unreachable** → abort with "mcp-atlassian is unavailable. Check the JIRA_URL and JIRA_PERSONAL_TOKEN environment variables."
...
- `mcp__atlassian__jira_get_issue` — fetch issue details and comments (mcp-atlassian). Reads comments for report detection (Tier 2) and duplicate check (Phase 3).
- `mcp__atlassian__jira_add_comment` — post aftersales script as comment on JIRA issue (mcp-atlassian)
- `delegate(role="executor", reasoning="medium")` — transformation subagent (Phase 4b)
...
```
Why good: Cleanly instructs user to run jira-analyze first. Does not attempt mid-execution skill invocation invocation.
</Good>
...
<Bad>
```
[Phase 3] No report found. Invoking $zaku:jira-analyze "SPFB-535"...
```
...
**Must NOT have:**
- Mid-execution `skill invocation` invocation of jira-analyze
