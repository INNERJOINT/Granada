...
**J3a. Download**: Call `mcp__atlassian__jira_download_attachments(issue_key=<KEY>)` only after Step J2 passes the all-attachment size gate. If the tool fails, returns no content, or returns an unusable structure, record a failed `MCP_DOWNLOAD` attempt with sticky reason `MCP_DOWNLOAD_FAILED` and proceed to Step J5.
