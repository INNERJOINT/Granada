...
- `mcp__atlassian__jira_download_attachments` accepts only an issue key, so call it only after metadata precheck confirms no attachment exceeds 50MB; otherwise skip MCP attachment download and use SN fallback or fail cleanly
...
Call `mcp__atlassian__jira_get_issue(issue_key=<KEY>, comment_limit=0)` to retrieve issue metadata. Store: title (summary), status, assignee, priority, description.
...
<Tool_Usage>
- `mcp__atlassian__jira_get_issue` — fetch issue details and attachment metadata (JIRA mode only, mcp-atlassian)
- `mcp__atlassian__jira_download_attachments` — download zip attachments as base64 (JIRA mode only, mcp-atlassian)
